...
public class LoaiSanPhamDAO extends SoftwareDAO<LoaiSanPham, String> {
	...
	public void insert(LoaiSanPham entity) {
		try {
			if (entity.getTenLoai().isEmpty() || entity.getMaLoai().isEmpty()) {
				return;
			}

			XJdbc.update(INSERT_SQL, entity.getMaLoai(), entity.getTenLoai(), entity.getMoTa());
	...
	public void update(LoaiSanPham entity) {
		try {
			...
			XJdbc.update(UPDATE_SQL, entity.getTenLoai(), entity.getMoTa(), entity.getMaLoai());
		} catch (SQLException ex) {
		}
	}
